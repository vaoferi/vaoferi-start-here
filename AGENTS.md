...
- `Trello` — лише legacy input: повністю прочитай релевантну картку, перенеси всі унікальні корисні факти й незавершену роботу в `Linear` або canonical docs/tests, після parity-перевірки архівуй картку.
