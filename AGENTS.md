...
- `Trello` — лише тимчасове legacy input для міграції: повністю прочитай релевантну картку, перенеси всі унікальні корисні факти й незавершену роботу в `Linear` або canonical docs/tests, перевір parity, після чого видали картку з Trello. Trello не використовується як planner або довгостроковий архів.
- Якщо доступний Trello-інструмент не підтримує hard-delete, не видавай archive за завершення cleanup: зафіксуй blocker/pending deletion і використай підтримуваний шлях видалення, коли він доступний.
